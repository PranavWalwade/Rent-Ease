  ...
  const handleInterestClick = async (propertyId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8110/TenantProperty/shortlist?userId=${userId}&propertyId=${propertyId}`,
        {
          method: "POST",
          headers: {
          ...
          >
            ...
            {Array.isArray(prop.photos) && prop.photos.length > 0 ? (
              <img
                src={
                  prop.photos[0]?.photo
                    ? `data:image/jpeg;base64,${prop.photos[0].photo}`
                    : "placeholder.jpg"
                }
                alt="Property"
                style={{
            ...
            <p>
              <strong>Rent:</strong> ₹{prop.rent || "N/A"}
            </p>
            <p>
              <strong>Lease Duration:</strong> {prop.leaseduration || "N/A"}
            </p>
            <p>
              <strong>Security Deposit:</strong> ₹
              {prop.securitydeposit || "N/A"}
            </p>
            <p>
              <strong>Size:</strong>{" "}
              {prop.sqfeet ? `${prop.sqfeet} sq. ft.` : "N/A"}
            </p>
            <p>
              <strong>Furnishing:</strong>{" "}
              {prop.furnishid?.furnishtype || "Not Specified"}
            </p>
            <p>
              <strong>Type:</strong>{" "}
              {prop.propertytypeid?.propertytypename || "Unknown"}
            </p>
            <p>
              <strong>Gas Connection:</strong>{" "}
              {prop.gasconnection ? "Yes" : "No"}
            </p>
            <p>
              <strong>Parking:</strong>{" "}
              {prop.parking ? "Available" : "Not Available"}
            </p>
            <p>
              <strong>Area:</strong> {prop.areaid?.areaname || "Unknown"}
            </p>

            {/* Owner Details */}
            <h4>Owner Details</h4>
            <p>
              <strong>Name:</strong> {prop.userid?.firstName}{" "}
              {prop.userid?.lastName}
            </p>
            <p>
              <strong>Contact:</strong> {prop.userid?.contact}
            </p>
            <p>
              <strong>Email:</strong> {prop.userid?.email}
            </p>
